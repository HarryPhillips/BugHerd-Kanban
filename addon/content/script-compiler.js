...
contentLoad: function(e) {
	...
	if (
		kanban_gmCompiler.isGreasemonkeyable(href)
...
injectScript: function(script, url, unsafeContentWin) {
	...
	// add our own APIs
	sandbox.GM_addStyle=function(css) { kanban_gmCompiler.addStyle(sandbox.document, css) };
	sandbox.GM_setValue=kanban_gmCompiler.hitch(storage, "setValue");
	sandbox.GM_getValue=kanban_gmCompiler.hitch(storage, "getValue");
	sandbox.GM_openInTab=kanban_gmCompiler.hitch(this, "openInTab", unsafeContentWin);
	sandbox.GM_xmlhttpRequest=kanban_gmCompiler.hitch(
...
hitch: function(obj, meth) {
	...
	return function() {
		if (kanban_gmCompiler.apiLeakCheck(hitchCaller)) {
			return;
		}
...
onLoad: function() {
	...
	if (appcontent && !appcontent.greased_kanban_gmCompiler) {
		appcontent.greased_kanban_gmCompiler=true;
		appcontent.addEventListener("DOMContentLoaded", kanban_gmCompiler.contentLoad, false);
	}
},

onUnLoad: function() {
	//remove now unnecessary listeners
	window.removeEventListener('load', kanban_gmCompiler.onLoad, false);
	window.removeEventListener('unload', kanban_gmCompiler.onUnLoad, false);
	window.document.getElementById("appcontent")
		.removeEventListener("DOMContentLoaded", kanban_gmCompiler.contentLoad, false);
},

}; //object kanban_gmCompiler


function kanban_ScriptStorage() {
	this.prefMan=new kanban_PrefManager();
}
kanban_ScriptStorage.prototype.setValue = function(name, val) {
